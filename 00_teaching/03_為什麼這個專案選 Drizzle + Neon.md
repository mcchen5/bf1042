...
也就是說，若課程設計上擔心「學生會不會比較難學」，這個擔心不是沒道理。
...
換句話說，學生不需要先切換到另一套專門的 ORM DSL，再回來理解 TypeScript 程式。整體心智模型比較一致。
...
這代表它不像某些 ORM 會把資料庫細節包得太深，而是讓學生仍然看得到：
...
Prisma 的 model 定義與 client 體驗，對很多學生來說比 SQL-first 風格更容易接受。
...
在這個過程中，讓學生更接近資料庫本體與 SQL 思維，比讓學生先得到一個很順手的 ORM API，更有長期價值。
...
## 7. 怎麼回應「學生會不會學不起來」這個擔心

這個擔心是合理的，但可以透過教學設計解掉，而不一定需要改選 Prisma。

比較精確地說：

- `Prisma` 比較會幫學生把很多細節藏起來
- `Drizzle` 則比較要求學生知道自己在定義什麼
...
只要這些基礎鷹架先存在，學生其實不需要從零硬背 Drizzle API，而是沿著既有範例修改。
...
### 代價三：課堂示範時要提醒學生「第一次慢是正常現象」

這點不難處理，但最好先說明，避免學生誤以為程式寫壞。
...
如果這門課後面要教的是 `Better Auth`，那學生很容易搞混：
